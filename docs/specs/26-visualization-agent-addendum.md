# Visualization Agent — 图表推荐与 Tableau 输出引导技术规格书
...
Visualization Agent 新增以下 3 个 MCP tools，追加入 Spec 26 §4 工具清单，总计由 32 个升至 **35 个**：
...
| Visualization Agent 阶段 | 对齐 Spec 26 Phase | 说明 |
|------------------------|------------------|------|
...
Visualization Agent 的入口集成在 Spec 14（NL-to-Query）查询结果面板的"可视化"按钮，不新建独立页面。调用链：
