...
> 版本：v0.4 | 状态：已审查（Gemini review，2 P1 + 3 P2 已修复） | 日期：2026-05-14 | 关联 PRD：待创建
>
> **变更记录**
> - v0.4（2026-05-14）— 新增指标编号 `metric_code` 系统生成规则；明确指标中文名必填、英文名降级为可选别名；新增派生/比率指标必须引用已注册指标的依赖闭环；强化 Tableau binding 为发布与查询强约束；新增 M-8 到 M-14 实施任务。
> - v0.3（2026-05-14）— 新增 Tableau published datasource 绑定、指标别名、派生指标结构化表达、`/api/metrics/lookup` 扩展契约，以及 Data Agent MetricResolver 接入计划。
...
### 2.3 Tableau Published Datasource 绑定（Data Agent P0）

Metrics Agent 必须支持将业务指标绑定到 Tableau published datasource。原因：首页 Data Agent 的生产问答以 Tableau MCP/VizQL 为执行层，`bi_data_sources.datasource_id` 无法表达 Tableau `connection_id + datasource_luid + fieldCaption` 的可查询边界。

新增绑定模型采用独立表，不直接复用 `bi_metric_definitions.datasource_id`：

| 表 | 说明 |
|----|------|
| `bi_metric_bindings` | 指标到执行数据源的绑定关系。一个指标可绑定多个执行源，但同一租户同一指标只能有一个 `is_primary=true` 的 Tableau 绑定。 |
| `bi_metric_aliases` | 指标别名表，用于中文、英文、业务简称、历史叫法匹配。 |

`bi_metric_bindings` 最小字段：

| 列名 | 类型 | 说明 |
|------|------|------|
| id | UUID | 主键 |
| tenant_id | UUID | 租户 ID |
| metric_id | UUID | FK → `bi_metric_definitions.id` |
| source_type | VARCHAR(32) | `database_table` / `tableau_published_datasource` |
| datasource_id | INTEGER NULL | 数据库数据源绑定，兼容现有模型 |
| tableau_connection_id | INTEGER NULL | Tableau connection id |
| tableau_asset_id | BIGINT NULL | 本地 `tableau_assets.id` |
| tableau_datasource_luid | VARCHAR(128) NULL | Tableau published datasource LUID |
| field_mappings | JSONB | 指标依赖到 Tableau fieldCaption 的映射，如 `{"sales": "销售额", "profit": "利润"}` |
| required_base_metrics | JSONB | 派生指标依赖的基础指标名，如 `["销售额", "利润"]` |
| formula_expression | JSONB | 结构化公式，禁止只存自然语言公式 |
| queryable_fields_snapshot | JSONB | 发布时的 MCP/VizQL 可查询字段快照，仅用于校验与审计 |
| is_primary | BOOLEAN | 是否默认执行绑定 |
| is_active | BOOLEAN | 是否启用 |
| created_at / updated_at | TIMESTAMP | 审计时间 |

`bi_metric_aliases` 最小字段：
...
| tenant_id | UUID | 租户 ID |
| metric_id | UUID | FK → `bi_metric_definitions.id` |
| alias | VARCHAR(128) | 别名，例如 `毛利率`、`profit margin` |
| locale | VARCHAR(16) | `zh-CN` / `en` / `mixed` |
| priority | INTEGER | 命中优先级 |
| is_active | BOOLEAN | 是否启用 |

约束：

1. Tableau 绑定的 `field_mappings` 中所有 fieldCaption 必须存在于绑定 datasource 当前 MCP/VizQL `queryable_fields`。
2. `ratio` / `derived` 指标必须声明 `required_base_metrics` 和 `formula_expression`，不得只依赖自然语言描述。
3. 首页 Data Agent 只能使用已发布、启用、且绑定到当前 Tableau connection/datasource 的指标定义。
4. 绑定失效时，Data Agent 必须返回“指标口径未绑定当前 Tableau 数据源”的标准 fallback，不得退回字段猜测。
5. 绑定表是指标治理到执行层的桥梁，不替代 `tableau_assets` 和 MCP 元数据同步。
6. 原子指标必须具备有效 Tableau binding 才能发布；没有 Tableau 资产依赖的指标只能作为草稿或待治理资产，不能进入可查询指标体系。
7. 派生指标与比率指标必须只引用 Metrics Registry 中已存在、同租户、未下线、且具备有效 Tableau binding 的原子或派生指标；禁止引用自由文本中的未知指标名。
8. 派生/比率指标若依赖跨 Tableau published datasource 的基础指标，默认拒绝保存；未来如需支持跨源计算，必须新增明确执行引擎契约后再放开。
...
| metric_code | VARCHAR(32) | NOT NULL, UNIQUE(tenant_id, metric_code) | 系统生成指标编号，例如 `MET-000001`；只作为稳定索引 key，不承载业务含义，不允许用户编辑 |
| name | VARCHAR(128) | NULLABLE | 指标英文名 / 技术别名；不再作为创建必填项，非空时仍需满足小写字母、数字、下划线规则 |
| name_zh | VARCHAR(256) | NOT NULL | 指标中文名，创建必填；用户主要通过中文名理解口径 |
...
| datasource_id | INTEGER | NULLABLE | 兼容旧数据库数据源字段；原子指标可填，派生/比率指标不再要求直接绑定物理表 |
| table_name | VARCHAR(128) | NULLABLE | 底层物理表名；仅 `atomic` 且 source_type=`database_table` 时必填 |
| column_name | VARCHAR(128) | NULLABLE | 底层物理字段名；仅 `atomic` 且 source_type=`database_table` 时必填 |
...
> ⚠️ **唯一约束**：`UNIQUE (tenant_id, metric_code)` 是唯一稳定 key；`name` 非空时建议保留 `UNIQUE (tenant_id, name)` 或 partial unique index，避免技术别名冲突；`name_zh` 必填但不作为绝对唯一 key，重名治理通过版本、业务域和别名管理完成。

### 3.2 bi_metric_dependencies（指标依赖关系表）

> ⚠️ **闭环依赖表**：派生指标和比率指标必须通过该表引用已注册指标，禁止引用指标体系之外的自由文本名称。

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | UUID | PK | 主键 |
| tenant_id | UUID | NOT NULL | 租户 ID |
| metric_id | UUID | NOT NULL, FK → bi_metric_definitions.id | 当前派生/比率指标 |
| depends_on_metric_id | UUID | NOT NULL, FK → bi_metric_definitions.id | 被引用的原子或派生指标 |
| dependency_role | VARCHAR(32) | NOT NULL | `base` / `numerator` / `denominator` |
| expression_order | INTEGER | NOT NULL DEFAULT 0 | 公式展示与计算顺序 |
| weight | FLOAT | NULLABLE | 预留：加权指标或组合指标的权重 |
| created_at | TIMESTAMP | NOT NULL DEFAULT now() | 创建时间 |

约束：

1. `metric_id != depends_on_metric_id`。
2. `metric_type=atomic` 不允许写入依赖记录。
3. `metric_type=derived` 至少 1 条 `dependency_role=base`。
4. `metric_type=ratio` 必须且只能有 1 条 `numerator` 和 1 条 `denominator`。
5. 写入依赖时必须校验引用指标同租户、未下线，且具备有效 Tableau binding。
6. 保存与发布时必须做 DAG 环路检测，禁止 A 依赖 B、B 又依赖 A 的循环口径。

### 3.3 bi_metric_lineage（血缘关系表）
...
### 3.5 bi_metric_anomalies（指标异常记录）
...
### 3.6 bi_metric_consistency_checks（一致性校验记录）
...
| bi_metric_definitions | ix_bmd_metric_code | (tenant_id, metric_code) | BTREE UNIQUE | 指标编号唯一索引 |
...
| bi_metric_definitions | ix_bmd_name | (tenant_id, name) | BTREE UNIQUE / PARTIAL | 非空英文别名唯一约束 |
| bi_metric_definitions | ix_bmd_sensitivity | (tenant_id, sensitivity_level) | BTREE | 敏感性级别筛选 |
| bi_metric_dependencies | ix_bmdp_metric | (tenant_id, metric_id) | BTREE | 查询指标依赖 |
| bi_metric_dependencies | ix_bmdp_depends_on | (tenant_id, depends_on_metric_id) | BTREE | 影响分析 |
...
erDiagram
    ...
    bi_metric_definitions ||--o{ bi_metric_dependencies : "metric_id"
    bi_metric_definitions ||--o{ bi_metric_dependencies : "depends_on_metric_id"
...
v0.4 后创建接口按 `metric_type` 分支校验，前端必须提交结构化 payload，后端不得从自由文本公式中猜测依赖。

**通用字段：**

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name_zh | string | 是 | 指标中文名 |
| name | string | 否 | 指标英文名 / 技术别名；非空时需唯一 |
| metric_type | string | 是 | `atomic` / `derived` / `ratio` |
| business_domain | string | 否 | 业务域 |
| description | string | 否 | 业务描述 |
| result_type | string | 否 | `float` / `integer` / `percentage` / `currency` |
| unit | string | 否 | 单位 |
| precision | integer | 否 | 小数位 |
| sensitivity_level | string | 否 | 敏感级别 |

**`atomic` 额外必填：**

| 字段 | 类型 | 说明 |
|------|------|------|
| tableau_connection_id | integer | Tableau 连接 ID |
| tableau_datasource_luid | string | Tableau published datasource LUID |
| tableau_asset_id | integer | 本地 Tableau asset ID |
| field_mappings | object | 原子指标到 Tableau fieldCaption 的映射，最小形态为 `{"value": "利润"}` |
| aggregation_type | string | `SUM` / `AVG` / `COUNT` / `COUNT_DISTINCT` / `MAX` / `MIN` / `none` |

**`derived` 额外必填：**

| 字段 | 类型 | 说明 |
|------|------|------|
| dependency_metric_ids | UUID[] | 依赖的原子或派生指标 ID，必须已注册且未下线 |
| formula_expression | object | 结构化公式，仅允许引用 `dependency_metric_ids` 中的指标 |

**`ratio` 额外必填：**

| 字段 | 类型 | 说明 |
|------|------|------|
| numerator_metric_id | UUID | 分子指标 ID，必须已注册且未下线 |
| denominator_metric_id | UUID | 分母指标 ID，必须已注册且未下线 |
| formula_expression | object | 默认可由后端生成 `divide(numerator, denominator)` |

创建成功响应必须返回系统生成的 `metric_code`。后端必须同时写入 `bi_metric_definitions`、`bi_metric_bindings`（如适用）、`bi_metric_dependencies`（如适用）和 `bi_metric_versions`。
...
| 参数 | 类型 | 位置 | 说明 |
|------|------|------|------|
| names | string | query | 逗号分隔的指标名或别名（e.g. `gmv,order_count,利润率`） |
| datasource_id | integer | query | 可选，限定数据库数据源 |
| tableau_connection_id | integer | query | 可选，限定 Tableau connection |
| tableau_datasource_luid | string | query | 可选，限定 Tableau published datasource LUID |
...
```json
{
  "metrics": [
    {
      "name": "profit_margin",
      "metric_code": "MET-000123",
      "name_zh": "利润率",
      "aliases": ["毛利率", "profit margin"],
      "metric_type": "ratio",
      ...
      "table_name": null,
      "column_name": null,
      "field_mappings": {"profit": "利润", "sales": "销售额"},
      "required_base_metrics": ["利润", "销售额"],
      "dependencies": [
        {"metric_code": "MET-000045", "name_zh": "利润", "role": "numerator"},
        {"metric_code": "MET-000046", "name_zh": "销售额", "role": "denominator"}
      ],
      "formula_expression": {
        "op": "divide",
...
1. `name`、`name_zh`、`bi_metric_aliases.alias` 三类命中。
2. 同名/同别名多指标时，优先选择绑定到当前 `tableau_connection_id + tableau_datasource_luid` 的指标。
3. 对 `derived` / `ratio` 指标返回依赖基础指标和结构化公式，供 Data Agent 生成 QuerySpec 与渲染派生列。
4. 返回结果不得包含未发布或已下线指标。
5. `not_found` 只表示指标名未命中；绑定存在但不可用必须返回 `binding_errors`，避免 Data Agent 误以为可用字段缺失。
...
sequenceDiagram
    ...
    participant MR as MetricResolver
    participant MA as Metrics Agent API
    participant DB as PostgreSQL

    DA->>MR: extract requested metric names from question
    MR->>MA: GET /api/metrics/lookup?names=利润率,销售额&tenant_id=xxx&tableau_connection_id=2&tableau_datasource_luid=...
    MA->>DB: SELECT active metric definitions + aliases + bindings
    DB-->>MA: [{name_zh: 利润率, formula_expression, field_mappings, ...}]
    MA-->>MR: 200 {metrics: [...], not_found: [], binding_errors: []}
    MR-->>DA: base_metrics + derived_metrics + display_contract

    Note over DA: QuerySpec 只下推基础聚合；派生指标由 renderer 基于 MCP 聚合结果计算
```

Data Agent 接入要求：

1. 普通 aggregate planning 前必须先调用 MetricResolver；MetricResolver 使用 `/api/metrics/lookup` 或同进程 service 查询指标定义。
2. QuerySpec 的 `metrics` 只包含 Tableau MCP 可执行的基础字段聚合。
3. QuerySpec 必须保留 `derived_metrics` 或等价 `params.derived_metrics`，用于语义覆盖校验和结果渲染。
4. 如果 Metrics Registry 命中完整指标定义，Data Agent 优先使用 registry 结果生成 deterministic QuerySpec。
5. 如果 registry 未命中，才允许进入 planning skill + LLM QuerySpec；LLM 输出仍必须通过 Validator。
6. 如果 registry 命中但当前 Tableau binding 不可用，必须返回标准 fallback，不得退回字段名猜测。

### 8.3 Metrics 能力提升计划与任务

| 阶段 | Task | 范围 | 验收 |
|------|------|------|------|
| P0 | M-1 指标别名与 lookup 扩展 | 增加别名匹配、中文名匹配、`binding_errors` | `利润率` / `毛利率` / `profit margin` 命中同一指标 |
| P0 | M-2 Tableau binding 模型 | 新增 `bi_metric_bindings`，支持 connection、asset、datasource_luid、field mappings | 指标可绑定到 `订单+ (示例 - 超市)` published datasource |
| P0 | M-3 派生指标结构化表达 | 支持 `required_base_metrics`、`formula_expression`、`result_type=percentage` | `利润率` 返回 `SUM(利润)/SUM(销售额)` 的结构化依赖 |
| P0 | M-4 Data Agent lookup contract | 输出 base metrics、derived metrics、display contract | Data Agent 不再从字段默认三件套猜指标 |
| P1 | M-5 UI 增强 | `/governance/metrics` 支持 Tableau datasource 选择、fieldCaption mapping、别名编辑 | 管理员可在 UI 完成指标与 Tableau datasource 绑定 |
| P1 | M-6 发布校验 | 发布时校验 field mappings 均在 MCP `queryable_fields` | 不可查询字段阻止发布 |
| P1 | M-7 回归集 | 维护 100+ 指标语义用例 | CI 覆盖原子、派生、比率、别名、绑定失效 |
| P0 | M-8 指标编号与主数据约束 | 新增系统生成 `metric_code`；`name_zh` 必填；`name` 改为可选技术别名 | 新建指标自动返回 `MET-xxxxxx`；不填写英文名也可创建 |
| P0 | M-9 指标依赖闭环 | 新增 `bi_metric_dependencies`；派生/比率只能引用已注册原子/派生指标 | 引用不存在指标时 422；详情页可展示依赖图 |
| P0 | M-10 Tableau binding 强校验 | 原子指标创建/发布必须绑定 Tableau published datasource 与字段；派生/比率继承依赖 binding 校验 | 无 Tableau binding 的指标不可发布、不可 queryable |
| P0 | M-11 类型化创建 API | `POST /api/metrics` 按 `atomic` / `derived` / `ratio` 分支校验并写入 bindings/dependencies | 原子、派生、比率三类创建路径均有后端测试 |
| P1 | M-12 指标表单重构 | 新建/编辑窗口按指标类型动态展示：原子选 Tableau 字段，派生选依赖指标，比率选分子分母 | 管理员无需填写英文名即可创建；利润率通过分子/分母建模 |
| P1 | M-13 lookup queryable 契约 | `/lookup` 返回 `metric_code`、`dependencies`、`queryable`、`binding_errors` | Data Agent 能区分“未注册”和“已注册但不可查询” |
| P1 | M-14 兼容迁移与数据治理 | 旧指标补 `metric_code`；`name_zh` 空值回填并标记待治理；无 binding 生成 binding_errors | 迁移 upgrade/downgrade 可逆；旧列表页不崩溃 |
