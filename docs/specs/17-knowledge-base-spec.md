...
```
RAG可用预算 = MAX(0, 2000 - data_context_actual)
```

其中 `data_context_actual` 按 `{字段数 × 平均字段描述 Token}` 动态计算（经验值：单字段约 30 Token）。

**兜底截断机制**：若 `data_context_actual`（如超大宽表）占用过大导致 RAG 预算不足时，必须触发 P0-P5 优先级截断机制：
- P0（术语精确匹配结果）：始终保留
- P1-P5：按优先级依次丢弃，**优先丢弃低频字段和低相关性（< 0.7）结果**，直到 RAG 预算恢复正值
