...
> **Scope 声明（T0.2 决策）**：本模块为**执行层**，负责流水线调度、执行、结果存储、信号灯判定。**规则定义层**由 [Spec 15](15-data-governance-quality-spec.md) 定义。两者职责边界：
> | 维度 | 本模块（Spec 31） | 规则定义层（Spec 15） |
> |------|------------------|---------------------|
> | 核心职责 | 流水线调度 + 执行 + 结果存储 | 规则定义 + CRUD + 评分模型 |
> | 数据表 | `bi_dqc_pipeline_*` | `bi_quality_rules` |
> | 调用关系 | 消费 Spec 15 规则定义 | 被消费方 |
