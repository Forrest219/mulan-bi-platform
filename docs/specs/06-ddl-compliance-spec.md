...
| 基于 rules.yaml 的静态规则引擎 | 基于 AST 的深度语义分析 |
| 数据库连接扫描全库表 | 跨库联合检查 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| ddl_text | string | 是 | CREATE TABLE SQL 语句 |
...
- **请求体大小**: 应限制 ddl_text 最大长度（建议 64KB），防止超大文本消耗资源
...
| PostgreSQL | SQLAlchemy ORM | bi_rule_configs / bi_scan_logs 表操作 |
| rules.yaml | 文件读取 | 启动时加载静态规则配置 |
...
| 编号 | 问题 | 影响范围 | 优先级 |
|------|------|----------|--------|
| OI-001 | 当前 DDLParser 基于正则表达式实现，对复杂 DDL（分区表、存储过程内嵌 DDL）解析能力有限。是否需要引入 SQL AST 解析器（如 sqlparse / sqlglot）？ | 解析覆盖率 | P2 |
| OI-002 | rules.yaml 静态规则与 bi_rule_configs 数据库规则为两套独立体系，DDL 检查使用 YAML，规则管理使用数据库。是否需要统一为单一规则源？ | 架构一致性 | P1 |
| OI-003 | 表级 COMMENT 在纯 SQL 文本解析模式下无法提取（仅支持数据库连接模式）。是否需要增强 Parser 支持 `COMMENT='xxx'` 语法？ | 检查完整性 | P2 |
| OI-004 | 当前评分权重（High=-20, Medium=-5, Low=-1）为硬编码。是否需要支持管理员自定义扣分权重？ | 灵活性 | P3 |
| OI-005 | DDL 检查目前不支持 ALTER TABLE 语句。是否需要扩展支持增量变更检查？ | 功能覆盖 | P2 |
| OI-006 | 规则变更审计表 bi_rule_change_logs 在当前代码中尚未实现写入逻辑。需要补充审计日志记录。 | 合规审计 | P1 |
| OI-007 | ddl_text 输入缺少长度限制，大文本可能导致正则引擎性能问题（ReDoS 风险）。需要增加输入校验。 | 安全 | P1 |
| OI-008 | 内置规则种子数据 (DEFAULT_RULES_SEED) 在模块导入时执行写入，若数据库未就绪会静默失败。是否需要改为显式初始化命令？ | 启动可靠性 | P2 |
