...
> **落地注记（2026-05-07）**：实际实现的端点为 `PUT /api/auth/change-password`（字段相同），
> 成功响应为 `{ "message": "密码修改成功，已退出其他设备" }`，错误响应为 `{ "detail": "旧密码不正确或用户不存在" }`。
...
> **落地注记（2026-05-07）**：修改密码以独立页面 `/account/password` 落地（非 settings 标签页），
> MFA 以独立页面 `/account/security` 落地。`/settings` 聚合入口暂未实现。
...
> **落地注记（2026-05-07）**：前端页面 `pages/account/password/page.tsx`，路由 `/account/password`；
> 密码强度校验：≥ 8 位（未做大小写+数字组合校验，与 AC-3 原要求有差异，后续可加强）。
