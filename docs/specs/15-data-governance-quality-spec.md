...
> ⚠️ **已取消（2026-05-06）**：本 Spec 对应的后端服务（`services/governance/`）和前端页面已全部删除。Spec 15 规则引擎由 Spec 31 DQC Pipeline 取代。本文件仅作历史存档，不再指导实现。

> **Scope 声明（T0.2 决策）**：本模块为**规则定义层**，负责质量规则库、规则模板、规则分类、规则 CRUD 管理。**执行层**（调度、流水线、产出）由 [Spec 31](31-governance-dqc-pipeline-spec.md) 定义。两者职责边界：
> | 维度 | 本模块（Spec 15） | 执行层（Spec 31） |
> |------|-----------------|------------------|
> | 核心职责 | 规则定义 + CRUD + 评分模型 | 流水线调度 + 执行 + 结果存储 |
> | 数据表 | `bi_quality_rules` | `bi_dqc_pipeline_*` |
> | 调用关系 | 被消费方 | 消费本模块规则定义 |
