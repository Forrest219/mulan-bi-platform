...
在执行 NL → VizQL 转换前，先从语义维护数据库中读取所有已维护的字段元数据，拼装为一段"数据字典文本"传给 LLM：

```
可用数据源和字段：
[数据源A: sales_db]
  - sales_amount (销售额, 单位:元, 指标口径:不含税销售收入, 同义词:收入/营收)
  - order_date (订单日期, 维度, 按自然月统计)
  - region (大区, 维度)
