...
| Tableau 智能资产中心 | v2.0 | Phase 2a~2c | 设计评审中 |
| DDL 规范检查 | — | 已完成 | 功能完成 |
| 规则配置 | — | 已完成 | 功能完成 |
| 数据库监控 | v0.2 | Phase 1 | 规划中 |
| AI 能力（搜索/解读） | v0.1 | 待评审 | 待开发 |
...
**PRD:** `docs/prd-tableau-v2.md` v2.0（设计评审中）

| 阶段 | 方向 | 核心功能 | 状态 |
|------|------|---------|------|
| Phase 2a | D1 深度 AI 解读 + D3 资产治理 | 同步增强、资产层级、深度AI解读、同步日志、定时同步 | 📋 规划中 |
| Phase 2b | D2 MCP 直连 + D3 治理增强 | 数据源元数据、NL-to-Query、视图截图、健康度评分 | 📋 规划中 |
| Phase 2c | D4 语义层融合 | 语义Schema自动生成、术语库、查询路由、RAG、AI标注 | 📋 规划中 |
...
**PRD:** `docs/prd-llm-layer.md` v0.1
...
- [ ] LLM Provider 统一配置（API Key + Base URL + Model）
- [ ] 标准化 LLM 调用服务（同步/流式）
- [ ] Tableau 资产生成摘要
- [ ] 报表详情页「AI 解读」按钮
