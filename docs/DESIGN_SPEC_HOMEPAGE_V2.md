...
>
> ⚠️ **2026-05-06 用户永久决策：`ConversationBar` 不再承载任何品牌区。**
> 外层主侧栏已经显示"木兰 BI 平台"，`ConversationBar` 内禁止再次渲染平台名、Logo、折叠按钮。
> `ConversationBar` 顶部只保留"新建对话"入口和搜索框，避免重复品牌区。
> 后端会话列表不得把已有用户消息的会话显示为默认标题"新对话"；当标题为空或仍为默认占位时，必须使用首条用户消息派生展示标题。
...
- [ ] `ConversationBar` 顶部不显示 logo、不显示"木兰平台"/"木兰 BI 平台"等品牌名、不显示折叠按钮；点击"新建对话"只回到 `/`，不创建空会话。
