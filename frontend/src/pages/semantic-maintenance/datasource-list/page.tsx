  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-2 mb-0.5">
            <i className="ri-translate-2 text-slate-500 text-base" />
            <h1 className="text-lg font-semibold text-slate-800">语义治理</h1>
          </div>
          <p className="text-[13px] text-slate-400 ml-7">Tableau 数据源语义管理与发布</p>
        </div>
      </div>
      ...
      <div className="px-8 py-7">
        ...
        {/* Connection Selector */}
        <div className="mb-4">
          <label className="block text-[12px] font-medium text-slate-600 mb-1.5">选择连接</label>
          <select
            value={selectedConnId || ''}
            onChange={e => { setSelectedConnId(Number(e.target.value)); setPage(1); }}
            className="px-4 py-2 border border-slate-200 rounded-lg text-[12px] focus:outline-none focus:border-blue-500 w-64"
          >
        ...
        <div className="flex items-center gap-4 mb-4">
          <label className="flex items-center gap-2 text-[12px] text-slate-600">
            ...
            <select
              ...
              className="px-3 py-1.5 border border-slate-200 rounded-lg text-[12px] focus:outline-none focus:border-blue-500"
            >
              {statusOptions.map(o => (
        ...
        {/* Table */}
        {loading ? (
          <div className="text-center py-12 text-slate-400 text-[13px]">加载中...</div>
        ) : loadError ? (
          <div className="text-center py-12 text-red-500 text-[13px]">{loadError}</div>
        ) : (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  {['数据源名称', '语义名称', '描述', '敏感级别', '状态', '发布状态', '更新时间', '操作'].map(h => (
              ...
              <tbody className="divide-y divide-slate-100">
                  ...
                  return (
                    <tr key={ds.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => navigate(`/governance/semantic/datasources/${ds.id}`)}>
                      <td className="px-4 py-3">
                        <div className="text-[12px] font-medium text-slate-800 hover:text-blue-600 transition-colors">{ds.asset_name || ds.tableau_datasource_id.slice(0, 20) + '…'}</div>
                        <div className="text-[11px] text-slate-400 font-mono">{ds.tableau_datasource_id.slice(0, 8)}…</div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-[12px] font-medium text-slate-800">{ds.semantic_name_zh || '-'}</div>
                        {ds.semantic_name && <div className="text-[11px] text-slate-400">{ds.semantic_name}</div>}
                      </td>
                      <td className="px-4 py-3 text-[12px] text-slate-600 max-w-xs truncate">{ds.semantic_description || '-'}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[11px] px-2 py-0.5 rounded-full ${sensBadge.className}`}>
                          {sensBadge.text}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-[11px] px-2 py-0.5 rounded-full ${statusBadge.className}`}>
                          {statusBadge.text}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {ds.published_to_tableau ? (
                          <span className="text-[11px] text-blue-600">已发布</span>
                        ) : (
                          <span className="text-[11px] text-slate-400">未发布</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-[11px]">{formatDate(ds.updated_at)}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 flex-wrap" onClick={e => e.stopPropagation()}>
                          <button
                            onClick={() => navigate(`/governance/semantic/datasources/${ds.id}`)}
                            className="px-2 py-1 text-[11px] text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded"
                          >
                            详情
                          </button>
                          <button
                            onClick={() => openEditModal(ds)}
                            className="px-2 py-1 text-[11px] text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded"
                          >
                            编辑
                          </button>
                          {ds.status === 'draft' || ds.status === 'ai_generated' || ds.status === 'rejected' ? (
                            <button
                              ...
                              className="px-2 py-1 text-[11px] text-amber-600 hover:text-amber-700 hover:bg-amber-50 rounded"
                            >
                              提交
                            </button>
                          ) : null}
                          {ds.status === 'reviewed' ? (
                            <>
                              <button
                                ...
                                disabled={actionLoading === ds.id}
                                className="px-2 py-1 text-[11px] text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded"
                              >
                                通过
                              </button>
                              <button
                                ...
                                disabled={actionLoading === ds.id}
                                className="px-2 py-1 text-[11px] text-red-600 hover:text-red-700 hover:bg-red-50 rounded"
                              >
                                驳回
                              </button>
                            </>
                          ...
                          {ds.status === 'approved' ? (
                            <>
                              <button
                                ...
                                className="px-2 py-1 text-[11px] text-purple-600 hover:text-purple-700 hover:bg-purple-50 rounded"
                              >
                                差异
                              </button>
                              <button
                ...
                {datasources.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center text-[12px] text-slate-400">
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] bg-violet-50 text-violet-600 rounded-lg hover:bg-violet-100 disabled:opacity-50 transition-colors"
              >
                {aiGenerating
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[12px] font-medium text-slate-600 mb-1.5">语义名称（中文）</label>
                <input
                  type="text"
                  value={editForm.semantic_name_zh || ''}
                  onChange={e => setEditForm({ ...editForm, semantic_name_zh: e.target.value })}
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-[13px] focus:outline-none focus:border-blue-500"
                  placeholder="如：月度销售额"
                />
              </div>
              <div>
                <label className="block text-[12px] font-medium text-slate-600 mb-1.5">语义描述</label>
                <textarea
              ...
              <div>
                <label className="block text-[12px] font-medium text-slate-600 mb-1.5">指标定义</label>
                <textarea
              ...
              <div>
                <label className="block text-[12px] font-medium text-slate-600 mb-1.5">维度定义</label>
                <textarea
              ...
              <div>
                <label className="block text-[12px] font-medium text-slate-600 mb-1.5">敏感级别</label>
                <select
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setEditingId(null)}
                className="px-4 py-2 text-[12px] text-slate-500 hover:text-slate-700">取消</button>
              <button onClick={handleSaveEdit}
                disabled={actionLoading === editingId}
                className="px-4 py-2 bg-slate-900 text-white text-[12px] font-medium rounded-lg hover:bg-slate-800 disabled:opacity-50">
                保存
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-lg">
                  <h3 className="text-[12px] font-medium text-slate-600 mb-2">Tableau 当前值</h3>
                  <pre className="text-[11px] text-slate-500 overflow-auto">
                    {JSON.stringify(showDiff.diff.tableau_current, null, 2)}
                  </pre>
                </div>
                <div className="p-4 bg-blue-50 rounded-lg">
                  <h3 className="text-[12px] font-medium text-blue-600 mb-2">Mulan 待发布值</h3>
                  <pre className="text-[11px] text-blue-700 overflow-auto">
              ...
              {Object.keys(showDiff.diff.diff || {}).length > 0 ? (
                <div className="p-4 bg-amber-50 rounded-lg">
                  <h3 className="text-[12px] font-medium text-amber-600 mb-2">差异内容</h3>
                  <pre className="text-[11px] text-amber-700 overflow-auto">
                    {JSON.stringify(showDiff.diff.diff, null, 2)}
                  </pre>
                </div>
              ) : (
                <div className="p-4 bg-emerald-50 rounded-lg text-[12px] text-emerald-600">
                  无差异，Tableau 当前值与 Mulan 待发布值一致
                </div>
              )}
              <div className="flex items-center gap-2">
                <span className={`text-[11px] px-2 py-1 rounded-full ${showDiff.diff.can_publish ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowDiff(null)}
                className="px-4 py-2 text-[12px] text-slate-500 hover:text-slate-700">关闭</button>
              {showDiff.diff.can_publish && (
                <button
                  onClick={() => { setShowDiff(null); handlePublish(showDiff.ds, false); }}
                  className="px-4 py-2 bg-blue-600 text-white text-[12px] font-medium rounded-lg hover:bg-blue-500"
                >
                  确认发布
                </button>
              )}
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[12px] text-slate-600 mt-1">{modalNotify.message}</p>
              </div>
            </div>
            <button onClick={() => setModalNotify(null)}
              className="mt-4 w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[12px] font-medium rounded-lg">
              关闭
            </button>
          </div>
        </div>
