...
/**
 * Smoke Test: 语义 - 数据源管理列表页
 * 路径：/governance/semantic/datasources（重定向到 /semantic-maintenance/datasources）
 */
test.describe('语义 - 数据源管理', () => {
  ...
  test('数据源语义管理页可访问', async ({ page }) => {
    await page.goto('/governance/semantic/datasources');
    await page.waitForLoadState('networkidle');
    expect(page.url()).toContain('/semantic-maintenance/datasources');
  ...
  test('页面标题和副标题可见', async ({ page }) => {
    await page.goto('/governance/semantic/datasources');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('h1')).toContainText('数据源语义管理', { timeout: 5000 });
    await expect(page.locator('text=管理和审核数据源语义描述')).toBeVisible();
  });
  ...
  test('有状态筛选下拉框', async ({ page }) => {
    await page.goto('/governance/semantic/datasources');
    await page.waitForLoadState('networkidle');
    const hasStatusFilter = await page.locator('text=状态筛选').isVisible().catch(() => false);
  ...
  test('/semantic-maintenance/datasources 路径可直接访问', async ({ page }) => {
    await page.goto('/semantic-maintenance/datasources');
    await page.waitForTimeout(2000);
    expect(page.url()).toContain('/semantic-maintenance/datasources');
