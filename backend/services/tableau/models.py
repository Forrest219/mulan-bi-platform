...
class TableauDatabase:
    ...
    def get_datasource_field_count(self, asset_id: int) -> int:
        """获取单个 datasource asset 的实际缓存字段数。"""
        return int(
            self.session.query(sa_func.count(TableauDatasourceField.id))
            .filter(TableauDatasourceField.asset_id == asset_id)
            .scalar()
            or 0
        )

    def update_asset_field_count(self, asset_id: int, field_count: int) -> bool:
        """修正单个 asset 的字段数量缓存。"""
        asset = self.get_asset(asset_id)
        if not asset:
            return False
        asset.field_count = field_count
        self.session.commit()
        return True
