...
AI_SEMANTIC_DS_TEMPLATE = """你是一个 BI 数据语义专家，请为以下 Tableau 数据源生成详细的业务语义建议。
...
## 输出要求（必须严格遵守）
请以 JSON 格式输出，所有标注"必填"的字段不得为空或 null：

- semantic_name: 英文语义名（驼峰命名，如 OrderDetail）
- semantic_name_zh: 中文语义名（必填，如"订单明细"）
- semantic_description: 语义描述（必填，2~3 句话：说明表的内容范围、包含的核心字段类型、主要业务用途）
- metric_definition: 指标口径（必填，列举 3~5 个可从该表聚合的核心指标，格式为"指标名：计算公式 → 业务含义"，每个指标单独一行）
- dimension_definition: 维度定义（必填，列举 4~6 个分析维度，格式为"维度名（字段来源）"，每个维度单独一行）
- business_definition: 业务定义（1~2 句话说明该表在业务流程中的位置和统计口径）
- sensitivity_level: 敏感级别（low/medium/high/confidential，订单/金额类通常为 high）
- tags_json: JSON 标签数组（如 ["销售", "订单", "明细"]）
